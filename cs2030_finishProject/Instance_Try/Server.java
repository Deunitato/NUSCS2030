...
class Server{
  ...
  /** Queue for customer waiting*/
	protected Queue<Event> myQueue = new LinkedList<>();
  /** Probability of server resting*/
	protected final double RESTING_PROBABILITY;
  /**
   ...
   */
	protected Server(int id,int queueSize){
		this.id = id;
   ...
   */
	protected Server(int id,int queueSize,double restingprob){
		this.id = id;
		this.MAX_QUEUE_SIZE = queueSize;
  ...
  */
	protected void addQueue(Event e){
		myQueue.add(e);
	}
   /**
  ...
  */
    protected boolean serveIncoming(Event event){
       ...
       if((incoming>=nextServeTime)){ //check if can serve
		   ...
		   double randomTime = EventSimulator.getServeTime(); //gen random serve time
  ...
  */
	protected boolean isFree(Event event){
  ...
  */
    protected boolean noWait(Customer arrival){
        double incoming = arrival.getArr();
		 if(canQueue()){ //checks if can queue
			this.waitTime = this.nextServeTime;
			//adds accordingly
            EventSimulator.addEvent(new Wait(incoming,arrival,this.id));
            EventSimulator.addBuffer(new Served(this.waitTime,arrival,this.id)); 
			addQueue(new Wait(incoming,arrival,this.id));
            return true;
		 }
		 else{
			 return false;
