...
private String plsShuffle(String str){
	...
	public Parser chop(String chopperString , int ... arr){
		
		Arrays.sort(arr);
		List <String> myList = new ArrayList<>();
		for(String s : myStringList){
			String  newString = ""; 
			if(!s.contains(chopperString)){
				myList.add(s);
				continue;
			}
			String[] stringArr = (s+"@").split(chopperString);
			...
			for(int i : arr){
				...
				if(first){
					newString  =  newString + stringArr[i-1];
					first = false;
				}
				else{
					newString = newString + chopperString + stringArr[i-1];
				}
			}
