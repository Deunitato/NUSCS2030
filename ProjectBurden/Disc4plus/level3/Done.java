...
public class Done extends Event{
	...
	//	if(EventSimulator.selfCheckExist()){
		if(super.isHuman()){
        return super.toString()+" "+ super.getCustomer().getID() + " done serving by server "+ super.getServer();
		}
		else{
			return super.toString()+" "+ super.getCustomer().getID() + " done serving by self-check "+ super.getServer();
		}
	/*	}
		else{
