	...
	*/
	public void TryRest(Event event){
		...
		if(this.es.genRestRate() < RESTING_PROBABILITY){ //rest
			...
			//System.out.println("Server  " + this.id+"  "+this.nextServeTime);
			//return false;
			if(!myQueue.isEmpty()){
	...
	*/
    public boolean serveIncoming(Event event){
       ...
       if((incoming>=nextServeTime)){ //check if can serve
		  ...
		  // TryRest(new Done(this.nextServeTime,event.getCustomer(),this.id));
		   
           return true;
       }
       else{
		   //System.out.println("iincoming failed" + this.id+"  "+this.nextServeTime + " for " +event.getCustomer().getID());
           return false;
       }
